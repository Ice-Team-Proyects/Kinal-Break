...
public class PasswordHashService : IPasswordHashService
{
    ...
    public bool VerifyPassword(string password, string hashedPassword)
    {
        try
        {
            // Limpiar escapes de barra invertida antes del símbolo de dólar (común en importaciones SQL)
            ...
            // Verificar si es el formato PBKDF2
            else if (hashedPassword.StartsWith("$pbkdf2$"))
            {
                Console.WriteLine("[DEBUG] Using PBKDF2 format verification");
                var result = VerifyPbkdf2Format(password, hashedPassword);
                Console.WriteLine($"[DEBUG] Verification result: {result}");
                return result;
            }
            else
    ...
    private bool VerifyPbkdf2Format(string password, string hashedPassword)
    {
        try
        {
            ...
            else if (algorithm.Equals("sha1", StringComparison.OrdinalIgnoreCase))
            {
        ...
        {
            Console.WriteLine($"Error verifying PBKDF2 format: {ex.Message}");
            return false;
        }
    }
